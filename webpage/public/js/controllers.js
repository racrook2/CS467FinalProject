	...
	function($scope, $http, $location, $anchorScroll) {
		...
		$http.get('./data/songs').success(function(data) {
			...
			for (var i = 0; i < $scope.songs.length; i++) {
				$scope.songs_sorted.push([$scope.songs[i]['name'], $scope.songs[i]['sentiment'], $scope.songs[i]['artist']]);
			}
		...
		$scope.findValue3 = function(song, artist) {
			for (var i = 0; i < $scope.songs_sorted.length; i++) {
				if ($scope.songs_sorted[i][0] == song) 
					if ($scope.songs_sorted[i][2] == artist) {
						// console.log(artist, $scope.songs_sorted[i][2]);
						return i;
					}
			}
			console.error("Song/artist not found")
			// console.log(artist);
			// console.log($scope.songs_sorted[0][2]);
			return -1;
		};
